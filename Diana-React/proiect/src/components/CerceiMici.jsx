...
function CerceiMici() {
  ...
  useEffect(() => {
    ...
    const filteredProducts = savedProducts.filter((product) =>
      product.denumire.includes("Cercei Mici")
    ); // Filtrează produsele care conțin "Cercei Mici" în denumire
    setProducts(filteredProducts); // Actualizează starea cu produsele filtrate
  }, []);

  const addToCart = (product) => {
    setCart([...cart, product]); // Adaugă produsul selectat în coș
    alert("Produs adăugat în coș!"); // Afișează un mesaj de confirmare
  };
